...
namespace jop
{
	...
	SoundStream::~SoundStream()
	{
		...
		for (int it = 0; it <m_bufferQueue.size(); ++it)
			...
			if (m_updatebuffer)
				{
					...
					if (m_fileInstance != NULL)
					{
						m_bufferQueue.back()->m_samples.clear();
							...
							{
								if (m_info.currentPos >= (m_fileInstance.getSize())&&m_playing)
									m_lastBuffer = true;
							...
							else
					    	{
							    ...
							    m_fileInstance.read(m_bufferQueue.back()->m_samples.data(), STREAMING_BUFFER_SIZE);
