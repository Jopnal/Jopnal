...
namespace jop
{
    ...
    RopeJoint& RopeJoint::enableAngMotor(const bool enable)
    {
        if (enable)
        {
            btVector3 lim(0.f, 0.f, 0.f);
            m_jointL->getLinearLowerLimit(lim);
            m_jointL->setLinearLowerLimit(lim * 0.99f);

            lim = btVector3(0.f, 0.f, 0.f);
            m_jointL->getLinearUpperLimit(lim);
            m_jointL->setLinearUpperLimit(lim * 0.99f);
        }
        else
        {
            btVector3 lim(0.f, 0.f, 0.f);
            m_jointL->getLinearLowerLimit(lim);
            m_jointL->setLinearLowerLimit(lim * 1.01f);

            lim = btVector3(0.f, 0.f, 0.f);
            m_jointL->getLinearUpperLimit(lim);
            m_jointL->setLinearUpperLimit(lim * 1.01f);
        }
        return *this;
    }
    /*
    RopeJoint& RopeJoint::enableMoveMotor(const bool enable)
    {
    m_jointL->setPoweredLinMotor(enable);
    return *this;
    }

    std::pair<float, float> RopeJoint::getAngLimits() const
    {
    return std::make_pair<float, float>(m_jointL->getLowerAngLimit(), m_jointL->getUpperAngLimit());
    }

    std::pair<float, float> RopeJoint::getAngMotorForces() const
    {
    return std::make_pair<float, float>(m_jointL->getTargetAngMotorVelocity(), m_jointL->getMaxAngMotorForce());
    }

    std::pair<float, float> RopeJoint::getMoveLimits() const
    {
    return std::make_pair<float, float>(m_jointL->getLowerLinLimit(), m_jointL->getUpperLinLimit());
    }

    std::pair<float, float> RopeJoint::getMoveMotorForces() const
    {
    return std::make_pair<float, float>(m_jointL->getTargetLinMotorVelocity(), m_jointL->getMaxLinMotorForce());
    }

    RopeJoint& RopeJoint::setAngLimits(const float min, const float max)
    {
    ...
    RopeJoint& RopeJoint::setAngMotorForces(const float speed, const float force)
    {
    ...
    RopeJoint& RopeJoint::setMoveLimits(const float min, const float max)
    {
    ...
    RopeJoint& RopeJoint::setMoveMotorForces(const float speed, const float force)
    {
