...
namespace detail
{
    struct FontImpl
    {
        stbtt_fontinfo m_font;                      ///< Font info
...
namespace jop
{
    ...
    bool Font::load(const int pixelSize)
    {
        ...
        {
            // Load & init
            stbtt_InitFont(&m_data.get()->m_font, reinterpret_cast<const unsigned char*>(m_data.get()->m_buffer.data()), 0/*stbtt_GetFontOffsetForIndex(buffer,0)*/); 
            // Scale
            m_data.get()->scale = stbtt_ScaleForPixelHeight(&m_data.get()->m_font, 15);
            // Get font metrics
            stbtt_GetFontVMetrics(&m_data.get()->m_font, &m_data.get()->ascent, 0, 0);
            // Baseline
            m_data.get()->baseline = (int)(m_data.get()->ascent * m_data.get()->scale);
            ...
            //// Box around all characters or just one?
            stbtt_GetFontBoundingBox(&m_data.get()->m_font, &bounds.first.x, &bounds.first.y, &bounds.second.x, &bounds.second.y);

            // Create stb rectangle
    ...
    std::pair<glm::vec2, glm::vec2> Font::getBounds(const int codepoint) const
    {
        ...
        int x, y, w, h;
        stbtt_GetGlyphBox(&m_data.get()->m_font, codepoint, &x, &y, &w, &h);
    ...
    {
        float k;
        k = stbtt_GetCodepointKernAdvance(&m_data.get()->m_font, codepoint1, codepoint2);
        return k;
    }
    ...
    void Font::getTextureCoordinates(const int codepoint, int* width, int* height, int* x, int* y) const
    {
        ...
        else
        {
            // Load glyph
            int glyph;
            glyph = stbtt_FindGlyphIndex(&m_data.get()->m_font, codepoint);
            // Glyph based functions > codepoint based, when performing multiple operations
            stbtt_GetGlyphBox(&m_data.get()->m_font, glyph, x, y, width, height);
            //
